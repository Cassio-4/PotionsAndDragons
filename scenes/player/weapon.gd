...
@onready var projectile_direction = $ProjectileDirection
...
func shoot():
	if attack_cooldown.is_stopped() and Bullet != null:
		...
		var direction = projectile_spawn.global_position.direction_to(projectile_direction.global_position).normalized()
